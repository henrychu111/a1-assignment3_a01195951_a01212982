...
async def fetch_pokedex_item_from_api(endpoint: str, request_data: list,
    ...
    pokedex_list = []
    async_coroutines = [get_pokedex_data(data_identification, endpoint, session)
    ...
    if endpoint == ABILITY_ENDPOINT:
        ...
        for response in responses:
            ...
            pokedex_list.append(Move(response["name"], response["id"], response["generation"]["name"],
                                     response["accuracy"], response["pp"], response["power"], response["type"]["name"],
...
async def fetch_pokemon_stat(base_stat: int, identification: str, session: aiohttp.ClientSession) -> list:
    response = await get_pokedex_data(identification, STAT_ENDPOINT, session)
...
async def execute_request(request: Request) -> list:
    ...
    async with aiohttp.ClientSession() as session:
        if request.mode == PokedexMode.POKEMON:
            async_coroutines = [get_pokedex_data(data_identification, POKEMON_ENDPOINT, session)
                                for data_identification in request_data]
            responses = await asyncio.gather(*async_coroutines)
            for response in responses:
                if request.expanded:
                    abilities = await fetch_pokedex_item_from_api(ABILITY_ENDPOINT,
                                                                  [ability["ability"]["name"] for ability in
                                                                   response["abilities"]],
                                                                  session)

                    moves = await fetch_pokedex_item_from_api(MOVE_ENDPOINT,
                                                              [move["move"]["name"] for move in response["moves"]],
                                                              session)

                    stats = [await fetch_pokemon_stat(stat["base_stat"], stat["stat"]["name"], session)
            ...
            pokedex_list = await fetch_pokedex_item_from_api(ABILITY_ENDPOINT, request_data, session)

        elif request.mode == PokedexMode.MOVE:
            pokedex_list = await fetch_pokedex_item_from_api(MOVE_ENDPOINT, request_data, session)
    return pokedex_list
